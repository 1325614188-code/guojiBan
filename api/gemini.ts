...
JSON 结构规范：
{
  ...
  "detailedAnalysis": "Markdown 格式的深度专业分析报告。必须严格、依次包含以下 7 个核心的行业标准检测维度及中英双语小标题，并使用专业术语进行细致阐述（不要夹杂除了以下7个标题外的其他一级或二级大标题，字数要求在400字以上，排版要优雅工整）：\n\n1. 种水 (Type and Water/Transparency)：[分析样品的种地，如玻璃种/冰种/糯种/豆种等，并评估其透明度（几分水）与晶体细腻致密度]\n2. 色泽 (Color)：[剖析颜色的分布特性，是否具有天然色根与渐变层次，颜色饱和度与是否属于邪色]\n3. 棉絮 (Cotton)：[评估内部絮状物、天然棉絮的多寡、结构形态及其对通透度和美观度的整体影响]\n4. 石纹 (Stone Veins) 与 裂纹 (Cracks)：[严格辨识是否带有天然石纹（无害）或由于外力及加工造成的破坏性微裂隙（有害）]\n5. 瑕疵 (Blemishes)：[分析黑点、脏点、杂质、共生矿物包裹体等多角度微观特征]\n6. 荧光反应 (Fluorescence/Glow) 与 起胶/起光 (Gelatinous/Luster Effect)：[深度探讨在折射/反射光下翡翠表面的起莹（荧）、起胶、刚性光泽表现，以判定内部结构的致密度及有无充胶B货迹象]\n7. 物理特性解释 (Physical Properties Explanation)：[用最专业的矿物物理学原理，例如解释致色元素铬（Cr）和铁（Fe）的均温分布特征、纤维交织结构的透光原理、天然玻璃光泽的硬度（莫氏硬度6.5-7）以及折射率（1.66）表现对成色的内在物理支持。]"
}`
