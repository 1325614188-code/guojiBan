...
const IQTestView: React.FC<IQTestViewProps> = ({ onBack, onCheckCredits, onDeductCredit, onResetLock }) => {
    ...
    }, [currentQuestion, answers, timeLeft, isCounting, showResult]);
    ...
    const getIQReport = () => {
        ...
        let level = t('tests.iq_lvl_average', '中等智力');
        let desc = t('tests.iq_desc_average', '你的思维能力处于大众平均水平。');
        let color = 'from-blue-400 to-sky-500';

        if (iqScore >= 140) {
            level = t('tests.iq_lvl_genius', '天才级别 (Genius)');
            desc = t('tests.iq_desc_genius', '你的智力表现极为显著，具备极强的逻辑构架和抽象思维能力，属于全球顶尖 1% 范畴。');
            color = 'from-purple-600 to-indigo-600';
        } else if (iqScore >= 120) {
            level = t('tests.iq_lvl_superior', '优秀 (Superior)');
            desc = t('tests.iq_desc_superior', '你具备出色的思维敏捷度和逻辑整合能力，解决复杂问题的效率远高于常人。');
            color = 'from-indigo-400 to-blue-500';
        } else if (iqScore >= 110) {
            level = t('tests.iq_lvl_high_average', '中上 (High Average)');
            desc = t('tests.iq_desc_high_average', '你能够较好地处理逻辑关系，认知能力稳健且具备较强的学习潜能。');
            color = 'from-blue-500 to-cyan-500';
        } else if (iqScore < 90) {
            level = t('tests.iq_lvl_potential', '仍有潜力 (Potential)');
            desc = t('tests.iq_desc_potential', '可能由于测试时的状态或偏好，你的表现略低于平均，建议多进行逻辑思维训练。');
        ...
        return (
            <div className="p-6 flex flex-col gap-6 bg-slate-50 min-h-screen">
                <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-xl font-bold">{t('tests.iq_report', 'IQ 智力测评报告')}</h2>
                </div>

                <div className={`bg-gradient-to-br ${report.color} rounded-[40px] p-8 text-white text-center shadow-lg`}>
                    <p className="text-xs uppercase tracking-widest opacity-80 mb-2">{t('tests.standard_iq_score', 'Standard IQ Score')}</p>
                ...
                <div className="bg-white rounded-3xl p-6 shadow-sm space-y-6">
                    <section>
                        <h3 className="text-slate-800 font-black mb-2 flex items-center gap-2">
                             <span className="w-1.5 h-1.5 rounded-full bg-blue-500"></span> {t('tests.assessment_interpretation', '测评解读')}
                        </h3>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                            <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">{t('tests.correct_count', '正确题数')}</p>
                            <p className="text-2xl font-black text-blue-600">{report.correctCount} / 60</p>
                        </div>
                        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                            <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">{t('tests.completion_time', '完成时间')}</p>
                            <p className="text-2xl font-black text-blue-600">{formatTime(2400 - timeLeft)}</p>
                        </div>
                    </div>

                    <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100">
                        <p className="text-[10px] text-amber-600 font-bold mb-1">💡 {t('tests.expert_advice', '专家建议')}</p>
                        <p className="text-xs text-amber-700 leading-relaxed italic">
                            {t('tests.iq_advice', '智力并非固定不变，通过深度的逻辑练习、跨领域学习和保持好奇心，认知边界仍可不断拓宽。')}
                        </p>
                    </div>
                </div>
        ...
        <div className="p-6 flex flex-col gap-6 bg-slate-50 min-h-screen">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="bg-white px-4 py-1.5 rounded-full shadow-sm border border-slate-100 flex items-center gap-2">
                    <span className="text-xs font-bold text-gray-400">{t('tests.time', 'Time')}:</span>
            ...
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100 relative overflow-hidden">
                <div className="flex justify-between items-end mb-2">
                    <div>
                        <p className="text-[10px] font-black text-blue-500 uppercase tracking-widest">{t('tests.question_en', 'Question')}</p>
                        <h3 className="text-2xl font-black text-slate-800">{currentIdx + 1} <span className="text-xs text-gray-300">/ 60</span></h3>
                    </div>
                    <div className="text-right">
                         <p className="text-[10px] font-black text-gray-400 uppercase">{t('tests.progress_en', 'Progress')}</p>
            ...
            <div className="bg-white rounded-[40px] p-8 shadow-xl shadow-slate-200/50 min-h-[200px] flex flex-col items-center justify-center text-center relative">
                 <div className="absolute top-6 left-1/2 -translate-x-1/2 px-3 py-1 bg-slate-50 rounded-full border border-slate-100">
                    <span className="text-[10px] lowercase font-bold text-slate-400 italic">{t('tests.type', 'type')}: {t(`tests.iq_type_${IQ_QUESTIONS[currentIdx].type}`, IQ_QUESTIONS[currentIdx].type)}</span>
                 </div>
                 <p className="text-[20px] font-bold text-slate-800 leading-relaxed mt-4">
                    {t(`tests.iq_q_${currentIdx + 1}`, IQ_QUESTIONS[currentIdx].q)}
                </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
                {IQ_QUESTIONS[currentIdx].options.map((opt, oIdx) => (
                    ...
                    >
                        {t(`tests.iq_q_${currentIdx + 1}_opt_${oIdx}`, opt)}
                    </button>
                ))}
            </div>
            ...
            <div className="flex gap-4 mt-auto pb-6">
                ...
                {Object.keys(answers).length === 60 ? (
                    ...
                    >
                        {t('tests.submit_results', 'Submit Results')}
                    </button>
                ) : (
                    <button
