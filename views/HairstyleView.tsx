...
const HairstyleView: React.FC<HairstyleViewProps> = ({ onBack, onCheckCredits, onDeductCredit, onResetLock }) => {
    ...
    try {
      ...
      const response = await fetch(getApiUrl('/api/gemini'), {
        ...
        body: JSON.stringify({
          ...
          hairstyleName: t(`tests.hair_${style?.id}_name`, style?.name || ''),
          hairstyleDesc: t(`tests.hair_${style?.id}_desc`, style?.desc || '')
        })
      });
  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-xl font-bold">{t('tests.hairstyle_reference', '发型参考')}</h2>
      </div>

      {/* 上传照片 */}
      <div className="flex flex-col gap-2">
        <p className="text-xs font-bold text-gray-500">1. {t('tests.upload_front_face', '上传正面人脸照片')}</p>
        <label className="aspect-[3/4] max-w-[200px] mx-auto rounded-2xl bg-white border-2 border-dashed border-gray-200 flex flex-col items-center justify-center overflow-hidden cursor-pointer">
          {faceImage ? (
            ...
            <>
              ...
              <span className="text-xs text-gray-400 mt-2 px-2 text-center">{t('tests.upload_clear_photo', '请上传清晰的正面照片')}</span>
            </>
          )}
      ...
      {/* 选择性别 */}
      <div className="flex flex-col gap-2">
        <p className="text-xs font-bold text-gray-500">2. {t('tests.select_gender', '选择性别')}</p>
      ...
      <div className="flex flex-col gap-3">
        <p className="text-xs font-bold text-gray-500">3. {t('tests.select_hairstyle', '选择发型风格')}</p>
          ...
          {hairstyles.map((style) => (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="font-bold text-xs">{t(`tests.hair_${style.id}_name`, style.name)}</span>
              </div>
              <p className="text-xs text-gray-500 line-clamp-2">{t(`tests.hair_${style.id}_desc`, style.desc)}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      >
        {loading ? (
          <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> {t('common.generating', '生成中...')}</>
        ) : t('tests.generate_hairstyle', '生成发型效果 💇')}
      </button>

      {/* 结果展示 */}
      {resultImage && (
        <div className="mt-4 flex flex-col gap-4">
          <p className="text-center font-bold text-gray-700">💇 {t('tests.your_hairstyle_result', '这是你的发型效果图：')}</p>
