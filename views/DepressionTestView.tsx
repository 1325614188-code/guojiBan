...
const DepressionTestView: React.FC<DepressionTestViewProps> = ({ onBack, onCheckCredits, onDeductCredit }) => {
    ...
    const getResultLevel = (score: number): { level: string; color: string; emoji: string; desc: string; advice: string } => {
        ...
        if (percentage <= 20) {
            return {
                level: t('tests.dp_lvl_healthy', '情绪健康'),
                color: 'from-green-400 to-emerald-500',
                emoji: '😊',
                desc: t('tests.dp_desc_healthy', '恭喜你！你的情绪状态非常健康，没有抑郁倾向。'),
                advice: t('tests.dp_adv_healthy', '继续保持积极乐观的生活态度，多运动、多社交、保持良好的作息习惯。')
            };
        } else if (percentage <= 40) {
            return {
                level: t('tests.dp_lvl_mild', '轻微情绪波动'),
                color: 'from-yellow-400 to-orange-400',
                emoji: '🙂',
                desc: t('tests.dp_desc_mild', '你有一些轻微的情绪波动，这是正常的生活压力反应。'),
                advice: t('tests.dp_adv_mild', '建议适当调整作息，增加运动和社交活动，培养一些兴趣爱好。如果持续感到困扰，可以与朋友倾诉。')
            };
        } else if (percentage <= 60) {
            return {
                level: t('tests.dp_lvl_moderate', '中度情绪困扰'),
                color: 'from-orange-400 to-red-400',
                emoji: '😔',
                desc: t('tests.dp_desc_moderate', '你可能正在经历一定程度的情绪困扰，需要关注自己的心理健康。'),
                advice: t('tests.dp_adv_moderate', '建议寻求家人朋友的支持，考虑咨询专业的心理咨询师。保持规律作息，适当运动，避免独处太久。')
            };
        } else if (percentage <= 80) {
            return {
                level: t('tests.dp_lvl_severe', '明显抑郁倾向'),
                color: 'from-red-400 to-red-600',
                emoji: '😢',
                desc: t('tests.dp_desc_severe', '你可能存在明显的抑郁倾向，建议尽快寻求专业帮助。'),
                advice: t('tests.dp_adv_severe', '强烈建议尽快预约专业心理医生或精神科医生进行评估和治疗。请告诉身边的亲朋好友你的感受，不要独自承担。')
            };
        } else {
            return {
                level: t('tests.dp_lvl_critical', '严重抑郁倾向'),
                color: 'from-red-600 to-purple-700',
                emoji: '🆘',
                desc: t('tests.dp_desc_critical', '你的测试结果显示可能存在严重的抑郁倾向，请立即寻求专业帮助。'),
                advice: t('tests.dp_adv_critical', '请立即联系心理危机干预热线（全国：400-161-9995）或前往医院精神科就诊。你不是一个人，专业帮助可以让你好起来。')
            };
        }
    };
        ...
        return (
            <div className="p-6 flex flex-col gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-xl font-bold">{t('tests.depression_result', '抑郁自测结果')}</h2>
                </div>
                ...
                <div className={`bg-gradient-to-br ${result.color} rounded-3xl p-6 text-white text-center`}>
                    ...
                    <p className="text-lg opacity-90">{t('tests.score_label', '得分：')}{totalScore} / {maxScore}</p>
                </div>

                <div className="bg-white rounded-2xl p-5 shadow-sm space-y-4">
                    <div>
                        <h3 className="font-bold text-gray-800 mb-2">📊 {t('tests.test_interpretation', '测试解读')}</h3>
                        <p className="text-gray-600 text-sm">{result.desc}</p>
                    </div>
                    <div>
                        <h3 className="font-bold text-gray-800 mb-2">💡 {t('tests.suggestions', '建议')}</h3>
                ...
                <div className="bg-pink-50 rounded-2xl p-4 border border-pink-200">
                    <p className="text-sm text-pink-700">
                        ⚠️ {t('tests.medical_disclaimer', '声明：本测试仅供参考，不能替代专业医学诊断。如有需要，请咨询专业心理医生。')}
                    </p>
                </div>

                <div className="bg-blue-50 rounded-2xl p-4 border border-blue-200">
                    <p className="text-sm text-blue-700 font-bold mb-1">🆘 {t('tests.mental_hotlines', '心理援助热线')}</p>
                    <p className="text-sm text-blue-600">{t('tests.hotline_1', '全国心理援助热线：400-161-9995')}</p>
                    <p className="text-sm text-blue-600">{t('tests.hotline_2', '北京心理危机研究与干预中心：010-82951332')}</p>
                </div>
    ...
    return (
        <div className="p-6 flex flex-col gap-6">
            <div className="flex items-center gap-4">
                <button onClick={onBack} className="text-2xl">←</button>
                <h2 className="text-xl font-bold">{t('tests.depression_test', '抑郁自测')}</h2>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm min-h-[120px] flex items-center justify-center">
                <p className="text-lg text-center font-medium text-gray-800">
                    {t(`tests.dp_q_${currentQuestion + 1}`, DEPRESSION_QUESTIONS[currentQuestion].q)}
                </p>
            </div>
            ...
            <div className="flex flex-col gap-3">
                {[
                    { score: 0, label: t('tests.freq_never', '完全没有'), color: 'bg-green-500' },
                    { score: 1, label: t('tests.freq_sometimes', '偶尔有'), color: 'bg-yellow-500' },
                    { score: 2, label: t('tests.freq_often', '经常有'), color: 'bg-orange-500' },
                    { score: 3, label: t('tests.freq_always', '几乎每天'), color: 'bg-red-500' },
                ].map(opt => (
                    <button
                        key={opt.score}
            ...
            <div className="flex gap-3">
                ...
                {allAnswered ? (
                    ...
                    >
                        {t('tests.view_results_chart', '查看结果 📊')}
                    </button>
                ) : (
                    <button
