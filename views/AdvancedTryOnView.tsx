...
const AdvancedTryOnView: React.FC<AdvancedTryOnViewProps> = ({ onBack, onCheckCredits, onDeductCredit, onResetLock }) => {
  ...
  const loadingTips = [
    t('tryon.loading_tip_1', 'AI 正在测量您的身形比例...'),
    t('tryon.loading_tip_2', '正在分析衣物材质与纹理...'),
    t('tryon.loading_tip_3', '光影渲染中，请稍候...'),
    t('tryon.loading_tip_4', '马上就好啦，魔法即将完成 ✨!')
  ];
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        try {
          const isValid = await detectPhotoContent(compressedBase64);
          if (!isValid) {
            alert(t('tryon.upload_hint', '检测建议：请上传带脸部的上半身正面清晰照片，以获得最佳试穿效果。'));
          }
          try { localStorage.setItem('tryon_avatar', compressedBase64); } catch(e) { console.warn('无法保存分身：', e); }
        } catch (error) {
          console.error('[AdvancedTryOnView] Detection error:', error);
          try { localStorage.setItem('tryon_avatar', compressedBase64); } catch(e) {}
        } finally {
  ...
  const handleCustomItemUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      try {
        const compressedBase64 = await compressImage(file, 1024, 0.8);
        setSelectedItemImage(compressedBase64);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (result && result === avatarImage) {
        onResetLock?.();
        alert(t('tryon.gen_failed', '生成效果与原图一致，判定为生成失败，本次不扣除使用次数，请尝试重新生成或更换图片'));
        return;
      }
      ...
      if (result) {
        ...
        onResetLock?.();
        alert(t('tryon.gen_failed', '生成失败，请稍后重试'));
      }
    } catch (e: any) {
      console.error(e);
      onResetLock?.();
      alert(`${t('tryon.gen_failed')}: ${e.message || 'Unknown Error'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteSavedLook = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (window.confirm(t('tryon.delete_confirm', '确定删除这个试穿记录吗？'))) {
    ...
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="bg-white px-4 pt-6 pb-2 shrink-0 shadow-sm z-10 sticky top-0">
        ...
        <div className="flex bg-gray-100 p-1 rounded-xl">
          ...
          >
            {t('tryon.tab_looks')} ({savedLooks.length})
          </button>
        </div>
      </div>
        ...
        {activeTab === 'create' && (
          <div className="flex flex-col gap-8 pb-10">
            ...
            <section>
              <div className="flex justify-between items-end mb-3">
                <h3 className="font-bold text-gray-800 text-lg flex items-center gap-2">
                  <span className="text-xl">👩</span> {t('tryon.my_avatar')}
                </h3>
                <label className="text-xs text-pink-500 font-bold bg-pink-50 px-3 py-1.5 rounded-full cursor-pointer active:scale-95 transition-transform">
                  {avatarImage ? t('tryon.re_upload') : t('tryon.upload_now')}
              ...
              <div className="bg-white rounded-3xl p-4 shadow-sm border border-gray-100 flex items-center gap-4">
                ...
                <div className="flex-1">
                  {avatarImage ? (
                    <>
                      <p className="font-bold text-gray-800 mb-1">{t('tryon.avatar_ready')}</p>
                      <p className="text-xs text-gray-500 leading-relaxed">{t('tryon.avatar_desc')}</p>
                    </>
                  ) : (
                    <>
                      <p className="font-bold text-gray-500 mb-1">{t('tryon.no_avatar')}</p>
                      <p className="text-xs text-gray-400">{t('tryon.upload_hint')}</p>
                    </>
                  )}
                </div>
              </div>
            ...
            <section>
              <div className="flex justify-between items-end mb-3">
                <h3 className="font-bold text-gray-800 text-lg flex items-center gap-2">
                  <span className="text-xl">👗</span> {t('tryon.pick_style')}
                </h3>
              ...
              {selectedItemImage && (
                <div className="mb-4 bg-white p-3 rounded-2xl shadow-sm border border-indigo-100 flex gap-4 items-center">
                   ...
                   <div className="flex-1">
                     <p className="font-bold text-indigo-700 text-sm">{t('tryon.selected_cloth')}</p>
                     <p className="text-xs text-gray-500 mt-1">{t('tryon.switch_style')}</p>
                   </div>
                </div>
              )}
            ...
            <div className="mt-4">
                   ...
                   {loading ? (
                    <div className="flex flex-col items-center">
                       <div className="flex gap-2 items-center">
                         ...
                         <span className="tracking-widest">{t('tryon.processing')}</span>
                       </div>
              ...
              {(!avatarImage || !selectedItemImage) && !loading && (
                 <p className="text-center text-xs text-gray-400 mt-3">{t('tryon.need_avatar_cloth')}</p>
              )}
            </div>
          ...
          <div className="pb-10">
            {savedLooks.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 opacity-50">
                ...
                <p className="text-gray-500 font-bold">{t('tryon.empty_lookbook')}</p>
                <p className="text-sm text-gray-400 mt-2">{t('tryon.try_new_clothes')}</p>
              </div>
            ) : (
