        ...
        return (
            <div className="p-6 flex flex-col gap-6 pb-12">
                <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-600 to-pink-600">{t('tests.talent_report', '职业天赋测试报告')}</h2>
                </div>
                ...
                <div className="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 rounded-[40px] p-8 text-white text-center shadow-xl shadow-purple-200 relative overflow-hidden">
                    ...
                    <div className="inline-block px-6 py-1.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30">
                        <span className="text-xl font-bold">{t(`mbti.desc_${result}_title`, desc.title)}</span>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-[32px] p-6 shadow-sm border border-purple-50">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <h3 className="font-black text-gray-800">{t('tests.detailed_personality_analysis', '深度性格解析')}</h3>
                    </div>
                    <p className="text-gray-600 text-sm leading-relaxed text-justify">
                        {t(`mbti.desc_${result}_detailed`, desc.detailedReport)}
                    </p>
                </div>
                ...
                <div className="bg-white rounded-[32px] p-6 shadow-sm border border-blue-50">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h3 className="font-black text-gray-800">{t('tests.core_competitiveness', '职场核心竞争力')}</h3>
                    </div>
                    
                    <div className="space-y-5">
                        <div>
                            <p className="text-[10px] font-black text-blue-500 uppercase mb-1">{t('tests.personality_talent', '性格天赋')}</p>
                            <p className="text-sm text-gray-700 font-medium">{t(`mbti.desc_${result}_traits`, desc.traits)}</p>
                        </div>
                        <div className="grid grid-cols-1 gap-4">
                            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                <p className="text-[10px] text-gray-400 font-bold mb-1">{t('tests.recommended_jobs', '推荐岗位')}</p>
                                <p className="text-xs text-gray-600 leading-relaxed font-bold">{t(`mbti.desc_${result}_careers`, desc.careers)}</p>
                            </div>
                        </div>
                        <div>
                            <p className="text-[10px] font-black text-blue-500 uppercase mb-1">{t('tests.career_advice', '职场避坑与建议')}</p>
                            <p className="text-sm text-gray-600 leading-relaxed italic">
                                {t(`mbti.desc_${result}_careerAdvice`, desc.careerAdvice)}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-[32px] p-6 shadow-sm border border-amber-100 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 mb-4 relative z-10">
                        ...
                        <h3 className="font-black text-amber-900">{t('tests.entrepreneurship_plan', '创业建议与大计划')}</h3>
                    </div>
                    <p className="text-amber-800 text-sm leading-relaxed font-medium relative z-10">
                        {t(`mbti.desc_${result}_entrepreneurship`, desc.entrepreneurship)}
                    </p>
                    <div className="mt-4 pt-4 border-t border-amber-200/50 flex justify-between items-center relative z-10">
                        <div>
                            <p className="text-[10px] text-amber-600 font-bold uppercase">{t('tests.earning_potential', '赚钱潜力评估')}</p>
                            <p className="text-sm">{(t(`mbti.desc_${result}_earning`, desc.earning).match(/^[^，,]*/)||[''])[0]}</p>
                        </div>
                        <div className="text-right">
                             <p className="text-[10px] text-amber-600 font-bold uppercase">{t('tests.industry_choice', '行业选择')}</p>
                             <p className="text-xs text-amber-900 font-bold">{(t(`mbti.desc_${result}_industries`, desc.industries).match(/^[^、,]*/)||[''])[0]}...</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-4 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                    <p className="text-[10px] text-gray-400 leading-relaxed">
                        {t('tests.mbti_disclaimer', '注意：本测试结果基于 MBTI 理论模型生成，旨在提供职业参考。个人发展受多种现实因素影响，请结合实际情况进行决策。')}
                    </p>
                </div>
                ...
                <button onClick={onBack} className="w-full h-16 bg-slate-900 text-white rounded-[24px] font-black shadow-xl active:scale-95 transition-all text-lg flex items-center justify-center gap-2">
                    <span>{t('common.home', '返回实验室首页')}</span>
    ...
    return (
        <div className="p-6 flex flex-col gap-6">
            <div className="flex items-center gap-4">
                ...
                <h2 className="text-xl font-bold">{t('tests.mbti_test', '职业天赋测试')}</h2>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm min-h-[120px] flex items-center justify-center">
                <p className="text-lg text-center font-medium text-gray-800">
                    {t(`tests.mbti_q_${currentQuestion + 1}`, MBTI_QUESTIONS[currentQuestion].q)}
                </p>
            </div>
            ...
            <div className="flex flex-col gap-3">
                {[
                    { score: 5, label: t('tests.opt_str_agree', '非常同意'), color: 'bg-purple-500' },
                    { score: 4, label: t('tests.opt_agree', '比较同意'), color: 'bg-purple-400' },
                    { score: 3, label: t('tests.opt_neutral', '一般'), color: 'bg-gray-400' },
                    { score: 2, label: t('tests.opt_disagree', '比较不同意'), color: 'bg-pink-400' },
                    { score: 1, label: t('tests.opt_str_disagree', '非常不同意'), color: 'bg-pink-500' },
                ].map(opt => (
                    <button
                        key={opt.score}
