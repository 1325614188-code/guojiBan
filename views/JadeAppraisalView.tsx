...
import { analyzeJadeImages } from '../services/jadeService';
...
const JadeAppraisalView: React.FC<JadeAppraisalViewProps> = ({ onBack, onCheckCredits, onDeductCredit }) => {
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (files.length + images.length > 6) {
            setError(t('failed')); // Or a specific max photo error if I added one
            return;
        }
    ...
    const startAnalysis = async () => {
        ...
        setError(null);
        try {
            const analysisResult = await analyzeJadeImages(images, lang);
        ...
        <div className="min-h-screen bg-[#f4f7f5] pb-20">
            ...
            <div className="flex items-center justify-between p-4 bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b border-emerald-100">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-bold text-gray-800">{t('jade_appraisal')}</h2>
                </div>
            ...
            <main className="p-4 space-y-6">
                ...
                <section className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                    ...
                    >
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-sm font-medium text-slate-700">{t('upload_hints')}</p>
                            <p className="text-[10px] text-slate-400">{t('jade_photos_hints')}</p>
                        </div>
                    </div>
                ...
                {!result && !isAnalyzing && !error && (
                    <div className="text-center p-10 bg-white/50 rounded-3xl border border-dashed border-slate-200">
                        ...
                        <p className="text-slate-400 text-sm">{t('report_placeholder')}</p>
                    </div>
                )}
                ...
                {result && !isAnalyzing && (
                    <div className="space-y-4">
                        <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                            <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">{t('appraisal_conclusion')}</span>
                            <h2 className="text-2xl font-bold mt-2 text-slate-900">{result.authenticity.conclusion}</h2>

                            <div className="grid grid-cols-2 gap-2 mt-4">
                                <div className="bg-slate-50 p-3 rounded-xl">
                                    <p className="text-[10px] text-slate-400 font-bold uppercase">{t('texture')}</p>
                                    <p className="text-sm font-bold">{result.quality.texture}</p>
                                </div>
                                <div className="bg-slate-50 p-3 rounded-xl">
                                    <p className="text-[10px] text-slate-400 font-bold uppercase">{t('transparency')}</p>
                                    <p className="text-sm font-bold">{result.quality.transparency}</p>
                                </div>
                                <div className="bg-slate-50 p-3 rounded-xl">
                                    <p className="text-[10px] text-slate-400 font-bold uppercase">{t('jade_color')}</p>
                                    <p className="text-sm font-bold">{result.quality.color}</p>
                                </div>
                                <div className="bg-slate-50 p-3 rounded-xl">
                                    <p className="text-[10px] text-slate-400 font-bold uppercase">{t('overall_grade')}</p>
                                    <p className="text-sm font-bold">{result.quality.overallGrade}</p>
                                </div>
                            </div>
                ...
                <section className="bg-emerald-900 text-emerald-50 rounded-3xl p-6">
                    ...
                    <ul className="space-y-3 text-xs opacity-80 list-disc pl-4">
                        {lang === 'zh' ? (
                            ...
                            <>
                                <li>Provide frontal photos in natural light to show the most realistic color.</li>
                                <li>Provide photos with strong transmitted light to help observe the internal structure.</li>
                                <li>Take macro photos of the surface to observe microscopic features like "fly wings".</li>
                            </>
                        )}
                    </ul>
                </section>
