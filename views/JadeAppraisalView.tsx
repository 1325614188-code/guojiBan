...
const JadeAppraisalView: React.FC<JadeAppraisalViewProps> = ({ onBack, onCheckCredits, onDeductCredit, onResetLock }) => {
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (files.length + images.length > 6) {
            setError(t('tests.upload_limit_6', "最多只能上传6张照片"));
            return;
        }
    ...
    const startAnalysis = async () => {
        ...
        try {
            ...
            if (analysisResult) {
                ...
                if (!deducted) {
                    throw new Error(t('common.error', "额度扣除失败，请稍后重试"));
                }
        ...
        <div className="min-h-screen bg-[#f4f7f5]">
            ...
            <div className="flex items-center justify-between p-4 bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b border-emerald-100">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-bold text-gray-800">{t('sections.jade_appraisal', '翡翠鉴别')}</h2>
                </div>
            ...
            <main className="p-4 space-y-6">
                ...
                <section className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                    <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
                        ...
                        {t('tests.upload_appraisal_photos', '上传鉴定照片')}
                    </h2>
                    ...
                    >
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-sm font-medium text-slate-700">{t('tests.click_to_upload', '点击上传照片')}</p>
                            <p className="text-[10px] text-slate-400">{t('tests.jade_upload_hint', '支持多角度、微距、透光照 (最多6张)')}</p>
                        </div>
                    </div>
                    ...
                    >
                        {isAnalyzing ? (
                            <>
                                ...
                                {t('ai.analyzing', '正在智能鉴定...')}
                            </>
                        ) : (
                            <>
                                <Search className="w-5 h-5" />
                                {t('ai.start_analysis', '开始严谨鉴定')}
                            </>
                        )}
                    </button>
                </section>
                ...
                {!result && !isAnalyzing && !error && (
                    <div className="text-center p-10 bg-white/50 rounded-3xl border border-dashed border-slate-200">
                        ...
                        <p className="text-slate-400 text-sm">{t('tests.upload_photo_to_analyze', '上传照片后，AI 将为您提供深度分析报告')}</p>
                    </div>
                )}
                ...
                <section className="bg-emerald-900 text-emerald-50 rounded-3xl p-6">
                    <h3 className="font-bold mb-4 flex items-center gap-2">
                        ...
                        {t('tests.jade_advice', '鉴定建议')}
                    </h3>
                    <ul className="space-y-3 text-xs opacity-80 list-disc pl-4">
                        <li>{t('tests.jade_advice_1', '提供自然光下的正面照，展示最真实的颜色。')}</li>
                        <li>{t('tests.jade_advice_2', '提供强光透射照，有助于观察内部结构。')}</li>
                        <li>{t('tests.jade_advice_3', '微距拍摄表面，观察“翠性”等微观特征。')}</li>
                    </ul>
                </section>
            </main>
        </div>
