...
const TryOnView: React.FC<TryOnViewProps> = ({ type, onBack, onCheckCredits, onDeductCredit, onResetLock }) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, isFaceImage: boolean) => {
      ...
      try {
        ...
        if (isFaceImage) {
          ...
          try {
            ...
            if (!isValid) {
              alert(t('tryon.upload_hint', '检测失败：需要上传带脸部的上半身正面照片（需露出肩膀和胸部）。'));
              setFaceImage(null);
            }
          } catch (error) {
  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-xl font-bold">{type === 'clothes' ? t('simple_tryon.title_clothes') : t('simple_tryon.title_accessories')}</h2>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div className="flex flex-col gap-2">
          <p className="text-xs font-bold text-gray-500">{t('simple_tryon.upload_upper')}</p>
          <label className="aspect-[3/4] rounded-2xl bg-white border-2 border-dashed border-gray-200 flex flex-col items-center justify-center overflow-hidden cursor-pointer">
            {faceImage ? (
              <div className="relative w-full h-full">
                ...
                {detecting && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/20 text-white">
                    ...
                    <span className="text-[10px] font-bold">{t('simple_tryon.detecting')}</span>
                  </div>
                )}
              </div>
            ) : (
              <>
                ...
                <span className="text-xs text-gray-400 mt-2 px-2 text-center">{t('simple_tryon.upload_upper_hint')}</span>
              </>
            )}
        ...
        <div className="flex flex-col gap-2">
          <p className="text-xs font-bold text-gray-500">{type === 'clothes' ? t('simple_tryon.upload_cloth') : t('simple_tryon.upload_ring')}</p>
      ...
      >
        {loading ? (
          <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> {t('simple_tryon.generating')}</>
        ) : t('simple_tryon.start_magic')}
      </button>

      {resultImage && (
        <div className="mt-4 flex flex-col gap-4">
          <p className="text-center font-bold text-gray-700">{t('simple_tryon.result_title')}</p>
