...
const BigFiveView: React.FC<BigFiveViewProps> = ({ onBack, onCheckCredits, onDeductCredit }) => {
    ...
    const calculateResult = () => {
        ...
        const analysis = {
            O: percentages.O > 70 ? t('tests.ocean_o_high', '高开放性：你充满好奇心，思想开放，敢于挑战权威和传统。') : percentages.O < 30 ? t('tests.ocean_o_low', '低开放性：你稳重踏实，更倾向于已有的、具体的、传统的事物。') : t('tests.ocean_o_mid', '中等开放性：你在创意和务实之间找到了很好的平衡点。'),
            C: percentages.C > 70 ? t('tests.ocean_c_high', '高尽责性：你极其自律，做事有条不紊，是极度可靠的成就者。') : percentages.C < 30 ? t('tests.ocean_c_low', '低尽责性：你随性洒脱，不喜欢被规则束缚，有时可能显得效率不高。') : t('tests.ocean_c_mid', '中等尽责性：你既能处理好工作，也能给自己留出灵活的空间。'),
            E: percentages.E > 70 ? t('tests.ocean_e_high', '高外向性：你从社交中获取能量，热衷交际，活力四射。') : percentages.E < 30 ? t('tests.ocean_e_low', '低外向性：你是一个内向者，通过独处恢复精力，享受深度安静。') : t('tests.ocean_e_mid', '中等外向性：你是混合型人格，在独处和社交之间切换自如。'),
            A: percentages.A > 70 ? t('tests.ocean_a_high', '高宜人性：你是一个利他主义者，善解人意，非常受周围人喜爱。') : percentages.A < 30 ? t('tests.ocean_a_low', '低宜人性：你竞争意识强，说话直爽，有时会显得有些冷漠。') : t('tests.ocean_a_mid', '中等宜人性：你懂得关心他人，但也能在必要时守住自己的原则。'),
            N: percentages.N > 70 ? t('tests.ocean_n_high', '高神经质：你情绪敏感，能够快速察觉环境变化，但也容易产生焦虑情绪。') : percentages.N < 30 ? t('tests.ocean_n_low', '低神经质（极稳）：你情绪极度稳定，抗压能力惊人，能够冷静面对各种挑战。') : t('tests.ocean_n_mid', '中等情绪波动：你的情绪状态处于大众普通水平。'),
        };
        ...
        return (
            <div className="p-6 flex flex-col gap-6 bg-[#fafafa] min-h-screen pb-10">
                <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-xl font-bold">{t('tests.ocean_report', 'OCEAN 人格分析报告')}</h2>
                </div>
                ...
                <div className="bg-gradient-to-br from-violet-600 to-indigo-500 rounded-[32px] p-8 text-white shadow-xl relative overflow-hidden">
                    ...
                    <p className="text-xs font-bold tracking-[0.3em] opacity-80 mb-2 uppercase">{t('tests.personality_insights', 'Personality Insights')}</p>
                    <h1 className="text-3xl font-black mb-4 leading-tight">{t('tests.discover_ocean_1', '发现你潜意识里的')}<br/>{t('tests.discover_ocean_2', '五大精神支柱')}</h1>
                ...
                <div className="bg-white rounded-3xl p-6 shadow-sm space-y-8">
                    ...
                    <div className="space-y-6">
                        {[
                            { key: 'O', label: t('tests.ocean_op', '开放性 (Openness)'), color: 'bg-indigo-500' },
                            { key: 'C', label: t('tests.ocean_co', '尽责性 (Conscientiousness)'), color: 'bg-emerald-500' },
                            { key: 'E', label: t('tests.ocean_ex', '外向性 (Extraversion)'), color: 'bg-amber-500' },
                            { key: 'A', label: t('tests.ocean_ag', '宜人性 (Agreeableness)'), color: 'bg-rose-500' },
                            { key: 'N', label: t('tests.ocean_ne', '情绪稳定性 (Neuroticism)'), color: 'bg-sky-500' },
                        ].map(trait => (
                            <div key={trait.key}>
        ...
        <div className="p-6 flex flex-col gap-6 bg-[#f8f9ff] min-h-screen">
            <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                    <h2 className="text-xl font-black text-violet-900 italic">{t('tests.ocean_test', 'Big Five / OCEAN')}</h2>
                    <p className="text-[10px] text-violet-300 font-bold tracking-widest uppercase">{t('tests.ocean_subtitle', '国际标准大五人格测试')}</p>
                </div>
            </div>
            ...
            <div className="flex justify-between items-center -mt-4">
                 ...
                 <span className="text-[10px] font-bold text-violet-400 uppercase">{t('tests.step', 'Step')} / 50</span>
            </div>
            ...
            <div className="bg-white rounded-[40px] p-10 shadow-xl shadow-violet-100/50 min-h-[220px] flex items-center justify-center text-center relative border border-violet-50">
                 <p className="text-[20px] leading-relaxed font-bold text-slate-800">
                    {t(`tests.bf_q_${currentIdx + 1}`, BIG_FIVE_QUESTIONS[currentIdx].q)}
                </p>
            </div>
            ...
            <div className="flex flex-col gap-3">
                {[
                    { score: 5, label: t('tests.eq_opt_very_match', '完全符合'), color: 'bg-violet-600' },
                    { score: 4, label: t('tests.eq_opt_match', '比较符合'), color: 'bg-violet-500' },
                    { score: 3, label: t('tests.opt_neutral_2', '中立 / 不一定'), color: 'bg-slate-400' },
                    { score: 2, label: t('tests.eq_opt_unmatch', '不太符合'), color: 'bg-slate-500' },
                    { score: 1, label: t('tests.eq_opt_very_unmatch', '完全不符'), color: 'bg-slate-600' },
                ].map((opt) => (
                    <button
                        key={opt.score}
            ...
            <div className="flex gap-4 mt-2">
                ...
                {allAnswered ? (
                    ...
                    >
                        {t('tests.explore_soul', 'EXPLORE MY SOUL')}
                    </button>
                ) : (
                    <button
