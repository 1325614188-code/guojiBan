...
// 拍摄场景定义
const SHOTS = [
    { id: 'center', labelKey: 'tests.eye_center', defaultLabel: '正视在前', descKey: 'tests.eye_center_desc', defaultDesc: '拨开上下眼皮，眼睛向前看', icon: '👁️' },
    { id: 'up', labelKey: 'tests.eye_up', defaultLabel: '仰视向上', descKey: 'tests.eye_up_desc', defaultDesc: '拨开下眼皮，眼睛往上看', icon: '⬆️' },
    { id: 'down', labelKey: 'tests.eye_down', defaultLabel: '俯视向下', descKey: 'tests.eye_down_desc', defaultDesc: '拨开上眼皮，眼睛往下看', icon: '⬇️' },
    { id: 'left', labelKey: 'tests.eye_left', defaultLabel: '左视向左', descKey: 'tests.eye_left_desc', defaultDesc: '拨开上下眼皮，眼睛往左看', icon: '⬅️' },
    { id: 'right', labelKey: 'tests.eye_right', defaultLabel: '右视向右', descKey: 'tests.eye_right_desc', defaultDesc: '拨开上下眼皮，眼睛往右看', icon: '➡️' }
];

const EyeDiagnosisView: React.FC<EyeDiagnosisViewProps> = ({ onBack, onCheckCredits, onDeductCredit, onResetLock }) => {
    ...
    const startAnalysis = async () => {
        if (images.some(img => img === null)) {
            setError(t('tests.upload_5_photos', "请完整上传5张照片"));
            return;
        }
        ...
        try {
            ...
            if (analysisResult) {
                ...
                if (!deducted) {
                    throw new Error(t('common.error', "额度扣除失败，请重试"));
                }
                setResult(analysisResult);
                setStep('result');
            } else {
                onResetLock?.();
                setError(t('common.error', "分析失败，未返回结果"));
                setStep('upload');
            }
        } catch (err: any) {
            console.error(err);
            onResetLock?.();
            setError(`${t('common.error', "分析失败")}: ${err.message || 'Unknown Error'}`);
            setStep('upload');
        }
    };
            ...
            <div className="min-h-screen bg-[#1a1b3b] text-white flex flex-col relative overflow-hidden">
                ...
                <div className="flex-1 flex flex-col items-center justify-center px-8 z-10 text-center">
                    ...
                    <h1 className="text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-blue-300 mb-4 tracking-wider">
                        {t('sections.ai_eye_diagnosis', 'AI 看眼')}
                    </h1>
                    <div className="bg-purple-900/40 backdrop-blur-sm border border-purple-500/30 px-6 py-2 rounded-full mb-8">
                        <p className="text-purple-100 text-sm font-medium">{t('tests.eye_slogan', '✨ 望眼知健康 ~')}</p>
                    </div>

                    <p className="text-gray-400 text-sm leading-relaxed mb-12">
                        {t('tests.eye_desc_1', '基于中医“五轮学说”理论')}<br />
                        {t('tests.eye_desc_2', '通过 5 个特定角度观察脏腑健康状态')}
                    </p>
                    ...
                    >
                        {t('tests.start_test_now', '立即检测')}
                    </button>
                    
                    <p className="text-[10px] text-gray-500 italic">
                        {t('tests.medical_disclaimer_short', '- 本程序结果仅供参考，不作为实际诊疗依据 -')}
                    </p>
                </div>
        ...
        return (
            <div className="min-h-screen bg-[#f8f9ff] flex flex-col">
                <div className="p-4 bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b flex items-center justify-between">
                    ...
                    <h2 className="font-bold text-gray-800">{t('tests.upload_photos', '上传照片')}</h2>
                    <div className="w-6" />
                </div>
                ...
                <main className="flex-1 p-6 space-y-6">
                    ...
                    <div className="bg-indigo-50 border border-indigo-100 rounded-3xl p-4 flex items-start gap-3">
                        ...
                        <div className="text-xs text-indigo-700 leading-relaxed">
                            <p className="font-bold mb-1 underline">{t('tests.photo_requirements', '拍摄环境要求：')}</p>
                            <p>{t('tests.eye_req_1', '· 光线充足，无遮挡，不要戴眼镜或隐形眼镜。')}</p>
                            <p>{t('tests.eye_req_2', '· 尽量由他人协助拍摄，确保对焦清晰。')}</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-[32px] p-8 shadow-sm border border-gray-100 text-center relative overflow-hidden">
                        ...
                        <h3 className="text-xl font-bold text-gray-900 mb-2">{t(SHOTS[currentShotIndex].labelKey, SHOTS[currentShotIndex].defaultLabel)}</h3>
                        <p className="text-gray-500 text-sm mb-8">{t(SHOTS[currentShotIndex].descKey, SHOTS[currentShotIndex].defaultDesc)}</p>
                        ...
                        >
                            ...
                            {images[currentShotIndex] ? (
                                ...
                                <>
                                    ...
                                    <p className="text-xs text-gray-400">{t('tests.click_to_upload', '点击拍照或上传')}</p>
                                </>
                            )}
                        </div>
                    ...
                    <div className="text-center space-y-1 py-4">
                        <p className="text-[10px] text-gray-400">{t('tests.privacy_notice', '-- 我们将保障您个人隐私安全，不会保留拍摄照片 --')}</p>
                    </div>
                </main>
            </div>
        );
        ...
        return (
            <div className="min-h-screen bg-[#1a1b3b] text-white flex flex-col items-center justify-center p-8 text-center">
                ...
                <div className="space-y-4">
                    ...
                    <h2 className="text-2xl font-bold tracking-tight">{t('ai.analyzing_eye', 'AI 智能望诊中')}</h2>
                    <p className="text-gray-400 text-sm">{t('ai.analyzing_eye_desc', '正在深度分析五轮特征，解读五脏六腑健康...')}</p>
                </div>
            ...
            <div className="min-h-screen bg-[#f0f2f9] flex flex-col pb-10">
                ...
                <div className="bg-[#5c4ae4] pt-12 pb-24 px-6 rounded-b-[48px] relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between text-white/80 mb-8 relative z-10">
                        ...
                        <h2 className="font-bold">{t('tests.health_report', '健康分析报告')}</h2>
                    ...
                    <div className="text-center text-white relative z-10">
                        <p className="text-white/60 text-sm font-medium mb-2 uppercase tracking-widest">{t('tests.health_score_en', 'Health Score')}</p>
                        <div className="text-7xl font-black mb-2 flex items-baseline justify-center">
                            {result.healthScore}
                            <span className="text-xl ml-1 font-bold text-white/50">{t('tests.points', '分')}</span>
                        </div>
                        <div className="flex items-center justify-center gap-2">
                             <div className={`px-4 py-1 rounded-full text-xs font-bold ${result.healthScore >= 80 ? 'bg-green-500/20 text-green-300' : 'bg-orange-500/20 text-orange-300'}`}>
                                {t('tests.status', '状态：')}{result.healthScore >= 80 ? t('tests.status_good', '良好') : result.healthScore >= 60 ? t('tests.status_fair', '一般') : t('tests.status_warn', '需预警')}
                             </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-[-80px] px-6 space-y-6 relative z-20">
                    <div className="grid grid-cols-3 gap-3">
                        <div className="bg-white rounded-3xl p-4 shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center">
                            ...
                            <p className="text-[10px] text-gray-400 font-bold mb-1">{t('tests.health_score_zh', '健康得分')}</p>
                            <p className="text-xs font-black text-gray-800">{result.healthScore}{t('tests.points', '分')}</p>
                        </div>
                        <div className="bg-white rounded-3xl p-4 shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center">
                            <div className="w-8 h-8 rounded-full bg-rose-50 flex items-center justify-center mb-2">
                                <Stethoscope className="w-4 h-4 text-rose-500" />
                            </div>
                            <p className="text-[10px] text-gray-400 font-bold mb-1">{t('tests.main_finding', '主要发现')}</p>
                        ...
                        <div className="bg-white rounded-3xl p-4 shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center">
                            ...
                            <p className="text-[10px] text-gray-400 font-bold mb-1">{t('tests.organe_check', '脏腑检测')}</p>
                            <p className="text-xs font-black text-gray-800">{t('tests.organe_desc', '五脏六腑')}</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-[32px] p-6 shadow-sm border border-gray-100">
                        <div className="flex items-center gap-2 mb-6">
                            ...
                            <h3 className="font-bold text-gray-900">{t('tests.wangyan_analysis', '望眼辩证分析')}</h3>
                        </div>
                    ...
                    <div className="bg-gray-100 rounded-2xl p-4 flex items-start gap-3">
                        ...
                        <p className="text-[10px] text-gray-400 leading-relaxed">
                            {t('tests.eye_medical_disclaimer_long', '本 AI 报告基于中医五轮学说理论生成，旨在提供一般性的健康参考，不能替代专业医师的面对面视诊与医疗建议。如您身体不适，请务必前往正规医疗机构就诊。')}
                        </p>
                    </div>
