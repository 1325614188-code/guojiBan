...
const EQTestView: React.FC<EQTestViewProps> = ({ onBack, onCheckCredits, onDeductCredit, onResetLock }) => {
    ...
    const translateDimension = (dim: string) => {
        const dict: Record<string, string> = {
            '自我意识': t('tests.eq_dim_self_awareness', '自我意识'),
            '自我管理': t('tests.eq_dim_self_management', '自我管理'),
            '自我激励': t('tests.eq_dim_self_motivation', '自我激励'),
            '共情能力': t('tests.eq_dim_empathy', '共情能力'),
            '社交技巧': t('tests.eq_dim_social_skills', '社交技巧')
        };
        return dict[dim] || dim;
    };
    ...
    const calculateResult = async () => {
        ...
        if (totalScore >= 200) {
            level = t('tests.eq_lvl_excellent', '卓越情商');
            analysis = t('tests.eq_analysis_excellent', '你具备极高的情绪智慧，是社交和职场中的领军人物。你不仅能完美掌控自己，还能深刻影响他人。');
            strengths = t('tests.eq_str_excellent', '洞察力极强、情绪稳定、具备强大的人格魅力和领导力。');
            weaknesses = t('tests.eq_weak_excellent', '有时可能因为过于完美而让周围人感到压力，或因顾及他人感受而牺牲过度。');
            suggestions = t('tests.eq_sug_excellent', '尝试在特定场合展现更为真实放松的一面，在决策中更平衡感性与理性的比例。');
        } else if (totalScore >= 160) {
            level = t('tests.eq_lvl_high', '高情商');
            analysis = t('tests.eq_analysis_high', '你的情商表现非常出色，能够游刃有余地处理大多数社交场合，情绪调节能力强。');
            strengths = t('tests.eq_str_high', '同理心旺盛、人际关系和谐、善于自我调节。');
            weaknesses = t('tests.eq_weak_high', '在极端压力下，偶尔会有情绪波动的风险。');
            suggestions = t('tests.eq_sug_high', '进一步深化对潜意识情绪的察觉，提升在高压环境下的情绪抗压能力。');
        } else if (totalScore >= 120) {
            level = t('tests.eq_lvl_medium', '中等情商');
            analysis = t('tests.eq_analysis_medium', '你具备基础的情绪管理意识，但在复杂的人际博弈或大幅情绪波动面前，仍有进步空间。');
            strengths = t('tests.eq_str_medium', '待人友善、具备一定的沟通技巧、工作态度积极。');
            weaknesses = t('tests.eq_weak_medium', '有时容易被情绪左右，或在社交中显得有些被动。');
            suggestions = t('tests.eq_sug_medium', '建议多进行正念冥想练习，提高自我察觉能力；学习更积极的沟通技巧，勇敢表达需求。');
        } else {
            level = t('tests.eq_lvl_needs_improvement', '情商待提升');
            analysis = t('tests.eq_analysis_needs_improvement', '你目前可能正处于情绪敏感期，或者在人际交往中遇到了一些阻碍，需要更多地关注内心世界。');
            strengths = t('tests.eq_str_needs_improvement', '内心单纯、执着、在特定领域专注力强。');
            weaknesses = t('tests.eq_weak_needs_improvement', '容易冲动、不擅长捕捉他人暗示、自我调节方式单一。');
            suggestions = t('tests.eq_sug_needs_improvement', '系统性地学习情绪管理课程；尝试每日记录情绪日记；多参加社交活动练习倾听。');
        }
        ...
        return (
            <div className="p-6 flex flex-col gap-6 bg-pink-50 min-h-screen pb-10">
                <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-xl font-bold">{t('tests.eq_report', '情商测试报告')}</h2>
                </div>

                <div className="bg-gradient-to-br from-pink-500 to-rose-400 rounded-[40px] p-8 text-white text-center shadow-xl">
                    <p className="text-sm opacity-90 mb-2 uppercase tracking-widest font-bold">{t('tests.eq_grade', 'Your EQ Grade')}</p>
                ...
                <div className="bg-white rounded-3xl p-6 shadow-sm space-y-6">
                    <section>
                        <h3 className="text-pink-600 font-black mb-3 flex items-center gap-2">
                             <span className="w-2 h-6 bg-pink-500 rounded-full"></span> {t('tests.overall_analysis', '总体分析')}
                        </h3>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        <section className="bg-green-50 p-4 rounded-2xl border border-green-100">
                            <h3 className="text-green-700 font-bold mb-2 flex items-center gap-2">✨ {t('tests.core_strengths', '核心优势')}</h3>
                        ...
                        <section className="bg-orange-50 p-4 rounded-2xl border border-orange-100">
                            <h3 className="text-orange-700 font-bold mb-2 flex items-center gap-2">⚠️ {t('tests.needs_attention', '需要注意')}</h3>
                    ...
                    <section>
                        <h3 className="text-pink-600 font-black mb-3 flex items-center gap-2">
                             <span className="w-2 h-6 bg-pink-500 rounded-full"></span> {t('tests.improvement_suggestions', '改进建议')}
                        </h3>
                    ...
                    <section>
                        <h3 className="text-pink-600 font-black mb-3 flex items-center gap-2">
                             <span className="w-2 h-6 bg-pink-500 rounded-full"></span> {t('tests.dimension_scores', '维度得分')}
                        </h3>
                            ...
                            {Object.entries(finalReport.dimensionScores).map(([key, value]: [any, any]) => (
                                <div key={key}>
                                    <div className="flex justify-between text-xs mb-1 font-bold">
                                        ...
                                        <span className="text-pink-500">{value}/50 {t('tests.score_unit', '分')}</span>
                                    </div>
                ...
                <button onClick={onBack} className="w-full h-16 bg-pink-500 text-white rounded-[24px] font-black shadow-lg active:scale-95 transition-all text-lg">
                    {t('tests.complete_and_start', '完成并开启魅力人生')}
                </button>
            </div>
        );
    }
        ...
        <div className="p-6 flex flex-col gap-6 bg-pink-50 min-h-screen">
            <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                    <h2 className="text-xl font-black text-gray-800">{t('tests.deep_eq_test', '深度情商测试')}</h2>
                    <p className="text-[10px] text-gray-400 font-bold tracking-widest uppercase">{t('tests.eq_assesment_en', 'Emotional Intelligence Assessment')}</p>
                </div>
            </div>
            ...
            <div className="space-y-2">
                <div className="flex justify-between items-end">
                    ...
                    <span className="text-[10px] font-black text-pink-300 uppercase tracking-tighter">{t('tests.processing_en', 'Processing...')}</span>
                </div>
            ...
            <div className="bg-white rounded-[40px] p-10 shadow-xl shadow-pink-100/50 min-h-[220px] flex flex-col items-center justify-center text-center relative overflow-hidden">
                 ...
                 <p className="text-sm text-pink-400 font-bold mb-4 uppercase tracking-[0.2em]">[{translateDimension(EQ_QUESTIONS[currentQuestion].dimension)}]</p>
                 <p className="text-[20px] leading-relaxed font-bold text-gray-800">
                    {t(`tests.eq_q_${currentQuestion + 1}`, EQ_QUESTIONS[currentQuestion].q)}
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3">
                {[
                    { score: 5, label: t('tests.eq_opt_very_match', '非常符合'), icon: '🌟' },
                    { score: 4, label: t('tests.eq_opt_match', '比较符合'), icon: '✨' },
                    { score: 3, label: t('tests.opt_neutral', '一般'), icon: '⚖️' },
                    { score: 2, label: t('tests.eq_opt_unmatch', '不太符合'), icon: '🌫️' },
                    { score: 1, label: t('tests.eq_opt_very_unmatch', '完全不符'), icon: '🌑' },
                ].map((opt) => (
                    <button
                        key={opt.score}
            ...
            <div className="flex gap-4 mt-4">
                ...
                {allAnswered ? (
                    ...
                    >
                        {t('tests.generate_deep_report', '生成深度报告 ✨')}
                    </button>
                ) : (
                    <button
