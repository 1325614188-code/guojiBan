const fs = require('fs');
const path = require('path');

const viewsDir = path.join(__dirname, 'views');
const files = fs.readdirSync(viewsDir).filter(f => f.endsWith('.tsx'));

const extracted = {};

// Static regex
const staticRegex = /t\(\s*['"`](tests\.[^'"`]+)['"`]\s*,\s*['"`]([^'"`]+)['"`]\s*\)/g;

files.forEach(file => {
    const code = fs.readFileSync(path.join(viewsDir, file), 'utf8');
    let match;
    while ((match = staticRegex.exec(code)) !== null) {
        extracted[match[1]] = match[2];
    }
});

fs.writeFileSync('extracted-static-keys.json', JSON.stringify(extracted, null, 2));
console.log('Extracted', Object.keys(extracted).length, 'static keys');
